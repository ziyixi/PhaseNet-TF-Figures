...
def main():
    ...
    pygmt.makecpt(cmap="jet", series=[0, 100, 1], continuous=True, reverse=False)
    fig.colorbar(
...
def load_catalog():
    ...
    # filter events between 0 to 100 km depth
    for i in range(len(catalogs)):
        catalogs[i] = catalogs[i][catalogs[i]["depth"] <= 100]
        catalogs[i].reset_index(drop=True, inplace=True)
