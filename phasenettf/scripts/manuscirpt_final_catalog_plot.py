...
def main():
    ...
    gcmt_line = project_gcmt(load_gcmt(), startlon, startlat, endlon, endlat)
    gcmt_line = gcmt_line[gcmt_line["event_id"].isin(["030994E", "201808190019A"])]
    if len(gcmt_line) > 0:
        fig.plot(
            x=gcmt_line["dist"] * degrees2kilometers(1),
            y=gcmt_line["dep"],
