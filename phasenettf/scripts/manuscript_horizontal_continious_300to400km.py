...
def main():
    ...
    pygmt.makecpt(cmap="jet", series=[300, 400, 1], continuous=True, reverse=True)
    fig.colorbar(
...
def load_catalog():
    ...
    # only keep 300-400km events
    for i in range(len(catalogs)):
        catalogs[i] = catalogs[i][catalogs[i]["depth"] < 400]
        catalogs[i] = catalogs[i][catalogs[i]["depth"] > 300]
